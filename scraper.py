...
def write_repos_to_markdown(filename, section_title, repos, is_subsection=False):
    ...
    with codecs.open(filename, "a", "utf-8") as f:
        ...
        for i, repo in enumerate(repos):
            ...
            f.write(
                f"| {i + 1} | [{repo['title']}]({repo['url']}) | {description} | {repo['language']} | {repo['stars']} | {repo['forks']} |\n")

        f.write("\n")


def scrape_all(use_timestamp=False,user_folder=None):
    """抓取所有数据并写入文件"""
    filename, date = get_file_path(use_timestamp, user_folder)
...
def job(use_timestamp=False, folder=None):
    """主函数，执行抓取任务"""
    try:
        print("开始抓取数据...")
        filename, display_date = scrape_all(use_timestamp, folder)
